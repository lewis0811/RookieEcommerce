...
namespace RookieEcommerce.Infrastructure.Persistence
{
    public class ProductRepository(ApplicationDbContext context) : BaseRepository<Product>(context.Products), IProductRepository
    {
        ...
        //private static readonly string productPrice = nameof(Product.Price).ToLowerInvariant();

        public Task<PaginationList<Product>> GetPaginated(GetProductsQuery query)
        {
        ...
        private static IQueryable<Product> ApplySort(GetProductsQuery query, IQueryable<Product> products)
        {
            ...
            //            case string s when s == productPrice:
            //                if (sortProduct != null)
            //                {
            //                    sortProduct = isDescending
            //                        ? sortProduct.ThenByDescending(c => c.Price)
            //                        : sortProduct.ThenBy(c => c.Price);
            //                    break;
            //                }
            //                sortProduct = isDescending
            //                    ? products.OrderByDescending(c => c.Price)
            //                    : products.OrderBy(c => c.Price);
            //                break;
            //        }
            //    }
